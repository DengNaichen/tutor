...
\begin{figure}[htp]
  ...
  \begin{tikzpicture}[scale=1.2, line width=1pt]
      % --- Left Graph: Constant Velocity ---
      \begin{scope}[shift={(0,0)}]
          % Axes
          \draw[->, thick] (0,0) node[below left] {$0$} -- (3.5,0) node[right] {$t$};
          \draw[->, thick] (0,0) -- (0,3) node[left] {$v$};
          
          % Velocity Line
          \draw[thick, blue] (0,1.8) node[left, text=black] {$v_i$} -- (2.5,1.8);
          
          % Dotted drop line
          \draw[thick, dotted, green!60!black] (2.5,1.8) -- (2.5,0) node[below, text=black] {$t_f$};
      \end{scope}

      % --- Right Graph: Uniform Acceleration ---
      \begin{scope}[shift={(5,0)}]
          ...
          % Velocity Line
          \draw[thick, blue] (0,0.8) node[left, text=black] {$v_i$} -- (2.5,2.3) node[above right, text=black] {$v_f$};
          
          % Dotted drop line
          \draw[thick, dotted, green!60!black] (2.5,2.3) -- (2.5,0) node[below, text=black] {$t_f$};
          
          % Slope Text
          \node at (0.9, 1.8) {\small $\text{slope} = a$};
          
          % Yellow visual "slice" (dt)
          \fill[yellow!60] (1.2, 0) rectangle (1.25, 1.52);
          \draw[orange, thin] (1.2, 0) rectangle (1.25, 1.52);
          \draw[orange, thin] (1.21, 0) -- (1.21, 1.52);
          \draw[orange, thin] (1.23, 0) -- (1.23, 1.52);
      \end{scope}
  \end{tikzpicture}
  \label{fig:v_t_diagram}
  \caption{$v-t$ diagram for uniform motion (left) and uniform acceleration motion (right). The thin yellow slice represents $v_{\text{slice}} \cdot dt$.}
\end{figure}
